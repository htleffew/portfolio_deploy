...
(function initGlobalChrome() {
    ...
    // EXCEPT when the URL carries a #fragment: anchor/index deep links must
    // land on their target, so never fight the browser's anchor positioning.
    ...
    /* updateChromeHrefs / executeScriptsQueue removed 2026-06-12 together with
       the SPA router (see note at former section 10). Navigation is native
       MPA again; chrome hrefs are computed once at injection time. */
    ...
    // 10. SPA Router — REMOVED 2026-06-12.
    //
    // The WebGL shader-transition router intercepted every internal link,
    // fetched the next page, ran a 1.2s HyperShader transition, and manually
    // re-executed the target page's scripts. Three user-facing failures traced
    // directly to it:
    //   (a) Navigation felt "very slow": every click paid fetch + 1.2s shader
    //       + full engine re-init before content appeared.
    //   (b) Re-injected page scripts that bind on DOMContentLoaded (e.g.
    //       library_dashboard.js) never ran after a swap — the Research
    //       Library arrived with dead accordions, filters, and search.
    //   (c) Re-running the reveal engines on the swapped DOM produced
    //       competing, misordered animations.
    // Navigation is now native MPA. Page-to-page polish is provided by the
    // CSS cross-document View Transitions already declared in
    // global_chrome.css (@view-transition { navigation: auto }), which are
    // instant, browser-managed, and degrade gracefully.
})();
