...
(function() {
  ...
  window.simMutate = function(type) {
    ...
    addLog('[ MUT-' + String(mutationCount).padStart(2,'0') + ' ] ' + msg + ' (−' + cost + ')', 'is-success');
    updateBar();
    if (budget <= 0) {
      halted = true;
      addLog('[ HALT ] MUTATION BUDGET EXHAUSTED. Autonomous loop terminated.', 'is-alert');
