...
const ResearchFeed = () => {
  ...
  return (
    <section id="research" style={{maxWidth:1100,margin:'0 auto',padding:'80px 48px 120px',borderTop:'1px solid rgba(51,51,51,0.3)'}}>
      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,letterSpacing:'0.25em',textTransform:'uppercase',color:'var(--phthalo-lift)',marginBottom:24}}>Methodology &amp; Documentation</div>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:12,justifyContent:'center',marginBottom:40}}>
        {CATS.map(c => (
          <button key={c} onClick={()=>setActive(c)}
            style={{background: active===c?'var(--phthalo-lift)':'transparent',border: active===c?'1px solid var(--phthalo-lift)':'1px dashed rgba(255,255,255,0.20)',
        ...
        {items.map((r, i) => (
          <a key={i} href="#" style={{display:'block',padding:'32px 0',borderBottom:'1px solid rgba(51,51,51,0.4)',textDecoration:'none'}}>
            <div style={{display:'flex',alignItems:'center',gap:16,marginBottom:12,fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:'var(--tungsten)',textTransform:'uppercase',letterSpacing:'0.10em'}}>
              <span style={{color:'var(--phthalo-lift)',border:'1px solid rgba(77,140,255,0.3)',padding:'4px 8px',borderRadius:4,background:'rgba(77,140,255,0.05)'}}>{r.cat}</span>
              <span>{r.time}</span>
            </div>
