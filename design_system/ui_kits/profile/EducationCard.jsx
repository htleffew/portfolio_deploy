...
const EducationCard = ({ degree, school, rows }) => (
  <div style={{position:'relative',background:'var(--charcoal)',border:'1px solid rgba(51,51,51,0.5)',borderRadius:4,overflow:'hidden'}}>
    ...
    <div style={{position:'relative',zIndex:1,padding:48}}>
      ...
      <p style={{fontFamily:"'Lora',serif",fontSize:16,color:'var(--sapphire)',fontStyle:'italic',margin:'0 0 24px'}}>{school}</p>
      {rows.map((r, i) => (
        <div key={i} style={{marginBottom:16}}>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:'var(--sapphire)',textTransform:'uppercase',marginBottom:8,letterSpacing:'0.10em'}}>{r.label}</div>
