...
- **Display (`--display`):** Georgia. Used exclusively for massive page titles (H1) and section headers (H2). A classic serif with strong presence—no external font imports required.
- **Body (`--body`):** Georgia. Used for all long-form prose and abstracts. Legible and elegant, set at `18px` with a `1.78` line-height. Standard web-safe serif.
- **Mono (`--mono`):** Courier New. Used for all utility text, eyebrows, metadata, and data visualizer labels. Always heavily tracked (`letter-spacing: 0.18em` to `0.30em`) and uppercase when used as labels. Universal monospace font.
...
## 5. The Master CSS Grid & Portrait Mode Polish (Global Layout Mandate)
...
## 6. Performance Mandates (The "Stutter" Fixes)
**CRITICAL:** 
