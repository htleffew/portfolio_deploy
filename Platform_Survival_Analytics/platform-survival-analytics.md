...
description: >-
  The Kaplan-Meier estimator asks "what is the probability that a patient is
  still alive at time t?" The same tool, with the same right-censoring
  assumptions, asks "what is the probability that a user session is still
  violation-free at time t?" A Spark-scale analytics engine that imports
  survival analysis from biostatistics into Trust & Safety, layered with LDA
  topic modeling, VADER sentiment, and NetworkX co-occurrence, producing a
  12-section auto-generated investigation report that is reproducible across
  cases and comparable across users.
