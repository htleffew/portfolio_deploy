...
\title{The Survival Function of Platform Integrity}
...
\end{abstract}
\section{Borrowing a tool from biostatistics}
...
\section{Four modules, one auditable pipeline}
...
\section{Survival analysis on user sessions}
The core analytical move is small in code and large in consequence. For each user-segment, the engine computes the temporal difference between consecutive content events as \texttt{duration} in seconds. The event indicator is one if the content interaction was associated with a flagged policy violation and zero otherwise. The pair (duration, event\textbackslash\{\}\_observed) is what \texttt{lifelines.KaplanMeierFitter} wants. Fit the estimator on the segment's data, plot the survival function, read off the median time to violation. The function is two dozen lines of Python; the reframing is the work.
\begin{verbatim}
...
\end{verbatim}
FIG. 02 The survival fit. Duration is the inter-event spacing in seconds; event\textbackslash\{\}\_observed is one when a policy was triggered. The median survival time is the population-level health metric.
...
\section{When did the topic neighborhood change?}
...
\section{Reproducibility through structured output}
